...
#include <App/Utilities/StackWalker.h>
...
public:
	...
	void LogSystemMessage( char const* _text, trace::TracedMessageType_t _type );
...
long OnCrash( _EXCEPTION_POINTERS* exceptionInfo )
{
	...
	fprintf( f, "REASON: %d\n", (uint32_t)exceptionInfo->ExceptionRecord->ExceptionCode );
...
{
	LogSystemMessage( msg, trace::TMT_Error );
...
void C_TraceServerApp::LogSystemMessage( char const* _text, trace::TracedMessageType_t _type )
{
	if ( m_Wnd != nullptr )	
		m_Wnd->LogSystemMessage( _text, _type );
}